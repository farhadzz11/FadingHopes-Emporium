...
def update_driver_info(driver_id: str) -> dict:
    while True:
        _result = show_a_driver(driver_id)
        ...
        match _user_choice:
            ...
            case "9":
                ...
                _result = execute_command("UPDATE employee SET employee_workExperience = %s "
                                          "WHERE person_id = %s", [str(_new_info), driver_id])
            ...
            case "10":
                ...
                if not _result["execute"]:
                    return {"update": False, "message": _result["message"]}
            case "11":
                _result = execute_command("SELECT driver_license FROM driver WHERE employee_id = %s",
                                          (driver_id,))
                ...
                while True:
                    _new_info = input(f"Please enter your new license (old -> {_old_info})\n"
                                      "If you want it to remain the previous value, enter 0\n>>> ")
                    if _new_info.lower() == "null":
                        print("\n\033[31mYou cannot use the phrase \"null\"\033[0m\n")
                        continue
                    ...
                    if _new_info not in ("A", "B", "C", "D", "E"):
                        print(
                            "\n\033[31mThe driver's license class you entered is invalid. Please enter one of the "
                            "following"
                            "classes:\nENTER A: This class allows you to drive any type of motor vehicle, including "
                            "cars,"
                            "trucks, and buses\nENTER B: This class allows you to drive cars and trucks, but not "
                            "buses\nENTER"
                            "C: This class allows you to drive motorcycles and scooters\nENTER D: This class allows "
                            "you to"
                            "drive taxis and other passenger vehicles\nENTER E: This class allows you to drive "
                            "vehicles that"
                            "tow trailers\033[0m\n")
                        continue

                    _result = execute_command("UPDATE driver SET driver_license = %s "
                                              "WHERE employee_id = %s",
                                              [_new_info, driver_id])
